        ...
        this.change = function (don) {
            ...
            if (don.tenkh != "" && don.sdt != "" && don.diachi != "") {
                ...
                for (var i_1 = 0; i_1 < this.xe.length; i_1++) {
                    if (this.xe[i_1].bienso == don.xe && don.xe != "") {
                        x = this.xe[i_1];
                    }
                }
                ;
...
var TaoDonHangComponent = (function () {
    function TaoDonHangComponent(dataService) {
        ...
        this.adddon = function (don) {
            console.log(don);
            if (don.tenkh != "" && don.sdt != "" && don.diachi != "" && this.items.length > 0) {
                var i = this.lengthdh + 1;
                var kh = { tenkh: don.tenkh, sdt: don.sdt, dc: don.diachi };
                var x;
                for (var i_1 = 0; i_1 < this.xe.length; i_1++) {
                ...
                this.isSuccess = true;
                this.msg = "Bạn đã thêm đơn hàng thành công!";
                this.chonxe = "Chọn xe giao hàng";
            }
            else {
                this.isSuccess = false;
                this.msg = "Bạn chưa nhập đủ thông tin khách hàng hoặc chưa có mặt hàng nào được thêm!";
            }
            this.dataService.countState(this.dataService.donhangs);
        };
        this.dataService.getMathangs().subscribe(function (mh) {
        ...
        this.dataService.getDonhangs().subscribe(function (donhangs) {
            ...
            for (var i = 0; i < donhangs.length; i++) {
                if (donhangs[i].trangthai == "Đã giao" && donhangs[i].id == _this.id) {
                    var storage = __WEBPACK_IMPORTED_MODULE_3_firebase__["storage"]().ref();
                    var path = "uploads/" + _this.donhangs[i].id + ".png";
...
var ViewDonhangComponent = (function () {
    function ViewDonhangComponent(route, dataService) {
        ...
        this.state = "";
        this.url = {};
        this.urls = [{ url: this.url, id: "" }];
        this.dataService.getDonhangs().subscribe(function (donhangs) {
            _this.donhangs = donhangs;
            var _loop_1 = function (i) {
                if (donhangs[i].trangthai == "Đã giao") {
                    storage = __WEBPACK_IMPORTED_MODULE_3_firebase__["storage"]().ref();
                    path = "uploads/" + donhangs[i].id + ".png";
                    spaceRef = storage.child(path);
                    storage.child(path).getDownloadURL()
                        .then(function (url) {
                        console.log("url", url);
                        var u = { url: url, id: donhangs[i].id };
                        // let img = this.imgsign.nativeElement;
                        // img.src = url;
                        //var test = url;
                        //(<HTMLImageElement>document.getElementById('imgsign')).src = test;
